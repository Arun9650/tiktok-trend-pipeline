...
## Finding target creators (audience seeding)

`src/fetchCreators.js` pulls trending creators from TikTok's Creative Center
via Apify, filtered to finance/banking-related bios and names.

```bash
npm run find-creators
```

This is **a watch list, not a repost source**. The output, `target-creators.json`,
is a list of accounts to follow and engage with (like, watch fully, comment)
so TikTok's algorithm learns your account belongs in that audience cluster.
Reposting their videos is a separate copyright problem regardless of who the
creator is, don't use this list for that.

**If it returns 0 relevant creators but found raw results,** the actor's
field names for creator data (follower count, bio, etc.) probably don't match
what `normalizeCreator()` in `fetchCreators.js` expects, Creative Center's
internal API isn't publicly documented so this was written defensively but
may need one adjustment. The script prints one raw creator object in that
case, paste it back and the field mapping is a two-line fix.
